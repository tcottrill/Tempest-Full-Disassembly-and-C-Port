...
#define KEY_NOCONVERT         VK_NONCONVERT
#define KEY_KANJI             VK_KANJI
#define KEY_BACKQUOTE         192

// Removed: KEY_YEN (125), KEY_AT (145), KEY_CIRCUMFLEX (144), KEY_COLON2 (146)
// and KEY_EQUALS_PAD (0x00). These were Allegro/DirectInput scancodes left in
// an otherwise virtual-key table, and as VK codes they alias live keys -
// 125 is VK_F14, 144 is VK_NUMLOCK, 145 is VK_SCROLL - so testing them fired
// on the wrong key. There is no virtual-key equivalent for them; use the
// scancode from WM_INPUT directly if you ever need these.
...
// A function, not a macro: the macro form evaluated its argument three times,
// so toUpper(*p++) advanced the pointer up to three times.
static __inline int toUpper(int ch)
{
	return (ch >= 'a' && ch <= 'z') ? (ch & 0x5f) : ch;
}
...
LONG GetMouseWheelChange(void);

//mouse button state checks
BOOL IsMouseLButtonDown(void);
BOOL IsMouseLButtonUp(void);
BOOL IsMouseRButtonDown(void);
BOOL IsMouseRButtonUp(void);
BOOL IsMouseMButtonDown(void);
BOOL IsMouseMButtonUp(void);
